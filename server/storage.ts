...
export interface IStorage {
  ...
  getAllProfiles(filters?: { industry?: string; search?: string; experienceLevel?: string }): Promise<Profile[]>;
  createProfile(profile: InsertProfile): Promise<Profile>;
  updateProfile(id: number, profile: Partial<InsertProfile>): Promise<Profile>;
  deleteProfile(id: number): Promise<void>;

  // Job operations
  getAllJobs(filters?: { industry?: string; search?: string; experienceLevel?: string }): Promise<Job[]>;
  ...
  getAllInternships(filters?: { department?: string; search?: string }): Promise<Internship[]>;
  ...
  getAllCvShowcase(filters?: { section?: string; search?: string }): Promise<CvShowcase[]>;
  ...
  updateCvShowcase(id: number, cv: Partial<InsertCvShowcase>): Promise<CvShowcase>;
  ...
  createCommunityBenefit(benefit: InsertCommunityBenefit): Promise<CommunityBenefit>;
  updateCommunityBenefit(id: number, benefit: Partial<InsertCommunityBenefit>): Promise<CommunityBenefit>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllProfiles(filters?: { industry?: string; search?: string; experienceLevel?: string }): Promise<Profile[]> {
    ...
    if (filters?.experienceLevel) {
      whereClause = and(whereClause, eq(profiles.experienceLevel, filters.experienceLevel))!;
    }
  ...
  // Job operations
  async getAllJobs(filters?: { industry?: string; search?: string; experienceLevel?: string }): Promise<Job[]> {
    ...
    if (filters?.experienceLevel) {
      whereClause = and(whereClause, eq(jobs.experienceLevel, filters.experienceLevel))!;
    }
  ...
  async getAllInternships(filters?: { department?: string; search?: string }): Promise<Internship[]> {
    let whereClause = and(
    ...
    if (filters?.department) {
      whereClause = and(whereClause, eq(internships.department, filters.department))!;
    }
  ...
  // CV Showcase operations
  async getAllCvShowcase(filters: { section?: string; search?: string } = {}): Promise<CvShowcase[]> {
    try {
      ...
      const result = conditions.length > 0 
        ? await query.where(and(...conditions)).orderBy(desc(cvShowcase.createdAt))
  ...
  async getCvShowcase(id: number): Promise<CvShowcase | undefined> {
    const [cv] = await db.select().from(cvShowcase).where(eq(cvShowcase.id, id));
    return cv;
  }

  async getCvByFilename(filename: string): Promise<CvShowcase | undefined> {
    const [cv] = await db.select().from(cvShowcase).where(eq(cvShowcase.cvFileName, filename));
    return cv;
  }
  ...
  async updateCvShowcase(id: number, cv: Partial<InsertCvShowcase>): Promise<CvShowcase> {
  ...
  async getAllWhitelistedEmails(): Promise<EmailWhitelist[]> {
    return await db.select().from(emailWhitelist).where(eq(emailWhitelist.isActive, true)).orderBy(desc(emailWhitelist.createdAt));
  }
  ...
  async addEmailToWhitelist(email: InsertEmailWhitelist): Promise<EmailWhitelist> {
  ...
  async bulkAddEmails(emails: InsertEmailWhitelist[]): Promise<EmailWhitelist[]> {
  ...
  async getStats(): Promise<{
    ...
    const [internshipCount] = await db
      .select({ count: sql<number>`count(*)` })
      .from(internships)
      .where(and(eq(internships.isActive, true), eq(internships.isApproved, true)));
  ...
  async getAllCommunityBenefits(): Promise<CommunityBenefit[]> {
    return await db.select().from(communityBenefits).orderBy(desc(communityBenefits.createdAt));
  }
  ...
  async getCommunityBenefit(id: number): Promise<CommunityBenefit | undefined> {
    const [benefit] = await db.select().from(communityBenefits).where(eq(communityBenefits.id, id));
    return benefit;
  }

  async createCommunityBenefit(benefit: InsertCommunityBenefit): Promise<CommunityBenefit> {
  ...
  async updateCommunityBenefit(id: number, benefit: Partial<InsertCommunityBenefit>): Promise<CommunityBenefit> {
  ...
  async findMatchingCVsForJob(jobId: number): Promise<CvShowcase[]> {
    ...
    const jobKeywords = this.extractKeywords(job.title + ' ' + job.industry + ' ' + (job.skills || ''));

    return allCVs.filter(cv => {
      const cvKeywords = this.extractKeywords(cv.name + ' ' + cv.title + ' ' + cv.section + ' ' + (cv.bio || ''));
      return this.calculateMatchScore(jobKeywords, cvKeywords) > 0.3; // 30% match threshold
    });
  }

  async findMatchingJobsForCV(cvId: number): Promise<{ job: Job; score: number }[]> {
    const cv = await this.getCvShowcase(cvId);
    if (!cv) return [];

    const allJobs = await this.getAllJobs();
    const cvKeywords = this.extractKeywords(cv.name + ' ' + cv.title + ' ' + cv.section + ' ' + (cv.bio || '') + ' ' + (cv.skills || ''));

    const jobMatches = allJobs.map(job => {
      const jobKeywords = this.extractKeywords(job.title + ' ' + job.industry + ' ' + (job.skills || '') + ' ' + (job.description || '') + ' ' + (job.requirements || ''));
  ...
  private extractKeywords(text: string): string[] {
    const relevantSkills = [
      // Technical Skills
      'javascript', 'typescript', 'react', 'node', 'python', 'java', 'php', 'sql', 'mongodb', 'mysql',
      'html', 'css', 'angular', 'vue', 'express', 'django', 'flask', 'spring', 'laravel', 'aws', 'azure',
      'docker', 'kubernetes', 'git', 'linux', 'windows', 'oracle', 'postgresql', 'redis', 'elasticsearch',
      
      // Business Skills  
      'marketing', 'sales', 'finance', 'accounting', 'hr', 'management', 'consulting', 'legal',
      'healthcare', 'engineering', 'design', 'operations', 'procurement', 'real estate', 'logistics',
      'manufacturing', 'construction', 'education', 'retail', 'hospitality', 'telecommunications',
      
      // Professional Levels
      'senior', 'junior', 'lead', 'head', 'director', 'coordinator', 'assistant', 'executive',
      'supervisor', 'team', 'project', 'product', 'strategy', 'business', 'technical',
      
      // Industry Terms
      'digital', 'technology', 'innovation', 'automation', 'communication', 'customer', 'service',
      'quality', 'safety', 'compliance', 'audit', 'risk', 'planning', 'analysis', 'research'
    ];
  ...
  async deleteEmailWhitelist(id: number): Promise<void> {
    ...
    try {
      ...
      const result = await client.query('DELETE FROM email_whitelist WHERE id = $1', [id]);
