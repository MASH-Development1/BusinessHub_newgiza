...
import { sendJobApplication, sendInternshipApplication, sendAccessRequestNotification, sendAccessApprovedNotification } from "./email";
import { insertJobSchema, insertInternshipSchema, insertCourseSchema, insertProfileSchema, insertApplicationSchema, insertCvShowcaseSchema, insertEmailWhitelistSchema, profiles, emailWhitelist } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const sessions = new Map<string, { email: string; isAdmin: boolean; timestamp: number }>();
    ...
    try {
      ...
      if (!isWhitelisted) {
        return res.status(403).json({ 
          message: "Access denied. Your email is not registered as a NewGiza resident. Please contact the community administrator." 
        });
      }
  ...
  app.post("/api/auth/cv-login", async (req, res) => {
    try {
      ...
      const userCv = userCvs.find(cv => cv.email.toLowerCase() === email.toLowerCase());
      
      if (!userCv) {
        return res.status(404).json({ message: "No CV found for this email address" });
      }
  ...
  app.post("/api/admin/jobs/:id/reject", async (req, res) => {
    try {
      const job = await storage.updateJob(parseInt(req.params.id), { status: 'rejected', isApproved: false });
      res.json(job);
    } catch (error) {
  ...
  app.get("/api/admin/internships", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.get("/api/admin/internships/pending", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch pending internships" });
    }
  });

  // Specific routes must come before parameterized routes
  app.get("/api/internships/pending", requireAuth, requireAdmin, async (req, res) => {
    try {
      const pendingInternships = await storage.getPendingInternships();
      res.json(pendingInternships);
    } catch (error) {
  ...
  app.post("/api/admin/internships/:id/approve", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid internship ID" });
  ...
  app.post("/api/admin/internships/:id/reject", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid internship ID" });
  ...
  app.patch("/api/internships/:id/approve", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid internship ID" });
  ...
  app.patch("/api/internships/:id/reject", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid internship ID" });
  ...
  app.post("/api/internships", requireAuth, async (req, res) => {
    try {
      ...
      const posterEmail = currentUser?.email || 
                         (process.env.NODE_ENV === 'development' ? 'admin@newgiza.com' : null);
      
      if (!posterEmail) {
        return res.status(401).json({ message: "Authentication required to submit internship" });
      }
      ...
      if (!internshipData.title || !internshipData.company || !internshipData.description || 
          !internshipData.posterRole || !internshipData.duration || !internshipData.contactEmail) {
        return res.status(400).json({ 
          message: "Missing required fields: title, company, description, posterRole, duration, and contactEmail are required" 
        });
      }
  ...
  app.post("/api/admin/internships/:id/approve", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const internship = await storage.updateInternship(id, { isApproved: true, status: "approved" });
      res.json(internship);
    } catch (error) {
      console.error("Error approving internship:", error);
      res.status(500).json({ message: "Failed to approve internship" });
  ...
  app.put("/api/admin/internships/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.delete("/api/admin/internships/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      await storage.deleteInternship(id);
  ...
  app.post("/api/profiles", requireAuth, upload.single('cv'), async (req, res) => {
    try {
      ...
      const profile = insertResult[0];
      console.log("Profile created successfully with ID:", profile.id, "for user:", userId);
  ...
  app.put("/api/profiles/:id", requireAuth, upload.single('cv'), async (req, res) => {
    try {
      ...
      const isOwnerByEmail = existingProfile.contact && currentUser.email && 
                           existingProfile.contact.toLowerCase() === currentUser.email.toLowerCase();
      
      const isOwnerByUserId = existingProfile.userId && currentUser.email && 
                             await storage.getUserByEmail(currentUser.email).then(user => 
                               user && user.id === existingProfile.userId
                             );
      ...
      if (req.file) {
        ...
        console.log("CV file uploaded for profile update:", req.file.filename);
      }
  ...
  app.delete("/api/profiles/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  // Application routes
  app.post("/api/applications/job/:jobId", upload.single('cv'), async (req, res) => {
    try {
      ...
      if (duplicateApplication) {
        return res.status(400).json({ 
          message: "You have already applied for this position. Duplicate applications are not allowed." 
        });
      }
      ...
      if (req.file && application) {
        try {
          ...
          console.log(`CV file registered for permanent retention: ${req.file.filename}`);
        } catch (error) {
  ...
  app.post("/api/applications/internship/:internshipId", upload.single('cv'), async (req, res) => {
    try {
      ...
      // Check for duplicate applications
      const existingApplications = await storage.getApplicationsByInternship(internshipId);
      const duplicateApplication = existingApplications.find(app => 
        app.applicantEmail.toLowerCase() === applicantEmail.toLowerCase()
      );
      ...
      if (duplicateApplication) {
        return res.status(400).json({ 
          message: "You have already applied for this internship. Duplicate applications are not allowed." 
        });
      }
  ...
  app.post("/api/cv-showcase", upload.single('cv'), async (req, res) => {
    try {
      ...
      const { name, email, phone, title, section, bio, skills, experience, education, yearsOfExperience, linkedinUrl } = req.body;
      ...
      if (req.file) {
        ...
        console.log(`✓ CV file ${req.file.filename} permanently stored with multiple protection layers`);
      }
  ...
  app.put("/api/cv-showcase/:id", upload.single('cv'), async (req, res) => {
    try {
      ...
      sessions.forEach((value, key) => {
        console.log(`  SessionID: ${key} -> Email: ${value.email}, IsAdmin: ${value.isAdmin}`);
      });
      ...
      const canEdit = session.isAdmin || session.email === existingCv.email.toLowerCase();
      console.log(`Can edit check: ${canEdit} (isAdmin: ${session.isAdmin}, emailMatch: ${session.email === existingCv.email.toLowerCase()})`);
      ...
      if (req.file) {
        ...
        if (existingCv.cvFileName) {
          const oldFilePath = path.join(process.cwd(), 'uploads', existingCv.cvFileName);
  ...
  app.patch("/api/cv-showcase/:id", async (req, res) => {
    try {
      ...
      // Check if user can edit this CV (only owner or admin)
      if (!session.isAdmin && session.email !== existingCv.email.toLowerCase()) {
        return res.status(403).json({ message: "Unauthorized to edit this CV" });
      }
  ...
  app.get("/api/cv/download/:filename", async (req, res) => {
    try {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        if (cvRecord) {
          const restoredFilename = await cvProtection.restoreCVFile(cvRecord.id);
          if (restoredFilename) {
            const restoredPath = path.join(process.cwd(), 'uploads', restoredFilename);
            if (fs.existsSync(restoredPath)) {
              console.log(`✓ CV file restored from protected copy: ${filename} -> ${restoredFilename}`);
              // Update the CV record with new filename
              await storage.updateCvShowcase(cvRecord.id, { cvFileName: restoredFilename });
        ...
        return res.status(404).json({ message: "CV file not found and could not be recovered" });
      }
  ...
  app.get("/api/cv-showcase/:id/download", async (req, res) => {
    try {
      ...
      if (!cv.cvFileName && !cv.cvFilePath) {
        ...
        return res.status(404).json({ message: "No CV file attached to this record" });
      }
      ...
      if (!actualFilename) {
        ...
        return res.status(404).json({ message: "CV file reference is corrupted" });
      }

      const filePath = path.join(process.cwd(), 'uploads', actualFilename);
      
      console.log(`Looking for CV file at: ${filePath}`);
      console.log(`CV record: ${cv.name} (${cv.email}) - cvFileName: ${cv.cvFileName}, cvFilePath: ${cv.cvFilePath}`);
      ...
      if (!fs.existsSync(filePath)) {
        ...
        await storage.updateCvShowcase(cvId, { 
          bio: cv.bio ? `${cv.bio}\n\n[Admin Note: CV file is missing from server]` : '[Admin Note: CV file is missing from server]'
        });
      ...
      try {
        ...
        return res.status(404).json({ message: "CV file is corrupted or unreadable" });
      }
      ...
      res.setHeader('Content-Disposition', `inline; filename="${cv.name}_CV${ext}"`);
  ...
  app.get("/api/admin/cv-integrity", requireAuth, requireAdmin, async (req, res) => {
    try {
      const allCvs = await storage.getAllCvShowcase();
      const uploadsDir = path.join(process.cwd(), 'uploads');
      const availableFiles = fs.readdirSync(uploadsDir).filter(f => f.startsWith('cv-'));
      ...
      const referencedFiles = allCvs.map(cv => cv.cvFileName).filter(Boolean);
      report.orphanedFiles = availableFiles.filter(file => !referencedFiles.includes(file));
  ...
  app.get("/api/admin/cv-protection/status", requireAuth, requireAdmin, async (req, res) => {
    try {
      const allCvs = await storage.getAllCvShowcase();
      ...
      try {
        ...
        totalSize = calculateDirSize(uploadsDir) + calculateDirSize(protectedDir) + calculateDirSize(backupDir);
      } catch (error) {
      ...
      const integrityScore = totalFiles > 0 ? Math.round((protectedFiles / totalFiles) * 100) : 100;
  ...
  app.get("/api/admin/cv-protection/files", requireAuth, requireAdmin, async (req, res) => {
    try {
      const manifestPath = path.join(process.cwd(), 'protected-cvs', 'PROTECTION_MANIFEST.json');
      ...
      const fileList = Object.entries(protectedFiles).map(([cvId, info]: [string, any]) => ({
        cvId: parseInt(cvId),
  ...
  app.post("/api/admin/cv-protection/verify", requireAuth, requireAdmin, async (req, res) => {
    try {
      const verification = await cvProtection.verifyProtectedFiles();
      res.json({
  ...
  app.post("/api/admin/cv-protection/backup", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/admin/disaster-recovery/backup", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { description } = req.body;
      const backup = await disasterRecovery.createCompleteBackup(description || 'Manual backup');
      res.json({
  ...
  app.get("/api/admin/disaster-recovery/backups", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/admin/disaster-recovery/restore/:backupId", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/admin/cv-cleanup", requireAuth, requireAdmin, async (req, res) => {
    try {
      const allCvs = await storage.getAllCvShowcase();
      const uploadsDir = path.join(process.cwd(), 'uploads');
      const cleanedCvs = [];
        ...
        if (cv.cvFileName) {
          ...
          if (!fs.existsSync(filePath)) {
            ...
            await storage.updateCvShowcase(cv.id, {
              cvFileName: null,
              cvFilePath: null,
              bio: cv.bio ? `${cv.bio}\n\n[File was missing and reference cleared by admin]` : '[File was missing and reference cleared by admin]'
            });
            cleanedCvs.push({
              id: cv.id,
              name: cv.name,
  ...
  app.get("/api/admin/cv-job-match/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const cvId = parseInt(req.params.id);
  ...
  app.delete("/api/whitelist/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/whitelist/import", excelUpload.single('excel'), async (req, res) => {
    try {
      if (!req.file) {
      ...
      if (emails.length === 0) {
        ...
        return res.status(400).json({ 
          message: "No valid emails found. Please ensure your Excel file has email addresses in a column labeled 'email', 'Email', or 'EMAIL'." 
        });
      }
  ...
  app.post("/api/auth/request-access", async (req, res) => {
    try {
      ...
      if (!fullName || !email || !unitNumber) {
        return res.status(400).json({ message: "Full name, email, and unit number are required" });
      }

      // Check if email is already whitelisted
      const isAlreadyWhitelisted = await storage.isEmailWhitelisted(email.toLowerCase());
      if (isAlreadyWhitelisted) {
        return res.status(400).json({ 
          message: "This email is already registered in the system. Please try logging in directly." 
        });
      }
      ...
      const hasPendingRequest = existingRequests.some(
        (request: any) => request.email.toLowerCase() === email.toLowerCase() && request.status === 'pending'
      );
      
      if (hasPendingRequest) {
        return res.status(400).json({ 
          message: "An access request for this email is already pending review. Please wait for admin approval." 
        });
      }
  ...
  app.get("/api/access-requests", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/access-requests/:id/approve", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/access-requests/:id/reject", requireAuth, requireAdmin, async (req, res) => {
    try {
      const requestId = parseInt(req.params.id);
  ...
  app.post("/api/admin/jobs/:id/approve", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const job = await storage.updateJob(id, { isApproved: true });
      res.json(job);
    } catch (error) {
      console.error("Error approving job:", error);
      res.status(500).json({ message: "Failed to approve job" });
  ...
  app.get("/api/admin/community-benefits", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/admin/community-benefits", requireAuth, requireAdmin, imageUpload.array('images', 5), async (req, res) => {
    try {
  ...
  app.put("/api/admin/community-benefits/:id", requireAuth, requireAdmin, imageUpload.single('image'), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.delete("/api/admin/community-benefits/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.get("/api/jobs/:id/matching-cvs", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.get("/api/cv-showcase/:id/matching-jobs", requireAuth, requireAdmin, async (req, res) => {
    try {
      const cvId = parseInt(req.params.id);
  ...
  app.post("/api/admin/change-credentials", requireAuth, requireAdmin, async (req, res) => {
    try {
  ...
  app.post("/api/admin/fix-whitelist", requireAuth, requireAdmin, async (req, res) => {
    try {
      await emailWhitelistFix.recreateTable();
      res.status(200).json({ message: "Email whitelist table recreated successfully" });
    } catch (error) {
      console.error("Error recreating table:", error);
      res.status(500).json({ message: "Failed to recreate table", error: error.message });
    }
  });

  // File integrity monitoring endpoint for admins
  app.get("/api/admin/file-integrity", requireAuth, requireAdmin, async (req, res) => {
    try {
