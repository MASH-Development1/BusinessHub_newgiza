...
export default function Admin() {
  ...
  const { data: removedJobs = [] } = useQuery({
    queryKey: ["/api/admin/removed-jobs"],
  });
  ...
  const createBenefitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Community benefit added successfully!",
      });
      setBenefitForm({
        title: "",
        description: "",
  ...
  const deleteBenefitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Community benefit deleted successfully!",
      });
    },
  });

  const restoreJobMutation = useMutation({
    mutationFn: async (removedJobId: number) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/removed-jobs"] });
      queryClient.invalidateQueries({ queryKey: ["/api/jobs"] });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Tabs defaultValue="requests" className="space-y-6">
          <TabsList className="grid w-full grid-cols-9">
            ...
            <TabsTrigger value="removed-jobs">Removed Jobs</TabsTrigger>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {Array.isArray(jobs)
                        ? jobs.filter((job: Job) => job.status === "pending")
                            .length
                        : 0}
                      )
                    </CardTitle>
                      ...
                      jobs.filter((job: Job) => job.status === "pending")
                        ...
                        jobs
                          ...
                          .map((job: Job) => (
                            <div
                              key={job.id}
                              className="border border-yellow-200 rounded-lg p-4 space-y-2 bg-yellow-50"
                            >
                              <div className="flex justify-between items-start">
                                ...
                                <div className="flex gap-2 ml-4">
                                  <Button
                                    ...
                                    onClick={() => {
                                      if (
                                        confirm(
                                          `Approve "${job.title}" from ${job.company}?`
                                        )
                                      ) {
              ...
              <Card className="mt-6">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    Active Jobs (
                    {Array.isArray(jobs)
                      ? jobs.filter((job: Job) => job.status === "active")
                          .length
                      : 0}
                    )
                  </CardTitle>
          ...
          <TabsContent value="removed-jobs" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  View and manage jobs that have been removed. You can restore
                  jobs if needed.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {removedJobs.length > 0 ? (
                    removedJobs.map((removedJob: any) => (
                      <div
                        key={removedJob.id}
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Originally posted:{" "}
                              {formatDate(removedJob.originalCreatedAt)}
                            </p>
                            <p className="text-sm text-muted-foreground">
                              Removed: {formatDate(removedJob.removedAt)}
                            </p>
                            {removedJob.removalReason && (
                              <p className="text-sm text-muted-foreground">
                                Reason: {removedJob.removalReason}
                              </p>
                            )}
                          </div>
                          <div className="flex gap-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                restoreJobMutation.mutate(removedJob.id)
                              }
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (
                                  confirm(
                                    `Delete "${internship.title}" internship?`
                                  )
                                ) {
                                  deleteInternshipMutation.mutate(
                                    internship.id
                                  );
                                }
                              }}
                        ...
                        >
                          <div className="flex justify-between items-start">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (
                                  confirm(`Delete "${course.title}" course?`)
                                ) {
                ...
                <CardContent>
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    {Array.isArray(benefits) && benefits.length > 0 ? (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                Added {formatDate(benefit.createdAt!)}
                              </p>
                            </div>
                      ...
                      <p className="text-muted-foreground text-center py-4">
                        No community benefits added yet
                      </p>
                    )}
                  </div>
                </CardContent>
                      ...
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (
                                confirm(`Delete "${profile.name}"'s profile?`)
                              ) {
