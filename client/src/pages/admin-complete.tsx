...
import { 
  ...
  DollarSign, Phone, User, Briefcase, GraduationCap, Edit, UserCheck,
  RotateCcw, Star, UserPlus, Save
} from "lucide-react";
...
export default function AdminComplete() {
  ...
  const [internshipForm, setInternshipForm] = useState({
    title: '', company: '', description: '', department: '', duration: '', requirements: ''
  });

  const [courseForm, setCourseForm] = useState({
    title: '', provider: '', description: '', type: '', duration: '', price: ''
  });
  ...
  const [userForm, setUserForm] = useState({
    name: '', email: '', phone: '', unit: '', isActive: true
  });
  ...
  const handleWhitelistSearchChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const createBackup = async () => {
    ...
    try {
      const response = await fetch('/api/admin/backups', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ description: `Manual backup - ${new Date().toLocaleString()}` }),
        credentials: 'include'
      });
      if (response.ok) {
  ...
  const restoreBackup = async (backupId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({ title: "Success", description: "Backup restored successfully. Refreshing data..." });
        setTimeout(() => {
  ...
  const updateUser = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setWhitelist(whitelist.map(u => u.id === editingUser.id ? updatedUser : u));
        setEditingUser(null);
  ...
  const downloadCV = async (filePath: string | undefined, fileName: string | undefined) => {
    if (!filePath || !fileName) {
      toast({ title: "Error", description: "CV file information is missing.", variant: "destructive" });
      return;
    }
    try {
      ...
      const response = await fetch(`/api/cv/download/${encodeURIComponent(actualFilename)}`, {
        credentials: 'include'
      });
    ...
    try {
      ...
      if (response.ok) {
        ...
        setWhitelist(prev => {
          ...
          if (request) {
            return [...prev, { id: Date.now(), email: request.email, addedAt: new Date().toISOString() }];
          }
          return prev;
        });
        toast({ title: "Success", description: "Access approved and added to whitelist" });
      }
    } catch (error) {
  ...
  const approveAllPendingRequests = async () => {
    ...
    if (pendingRequests.length === 0) {
      toast({ title: "No Action Needed", description: "No pending requests to approve" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Error", description: "Failed to approve all requests", variant: "destructive" });
    }
  };
  ...
  const deleteInternship = async (id: number, isApproved: boolean = false) => {
    try {
      ...
      if (response.ok) {
        ...
        if (isApproved) {
          setApprovedInternships(prev => prev.filter(internship => internship.id !== id));
        } else {
  ...
  const approveInternship = async (id: number, event?: React.MouseEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "Success", description: "Internship approved successfully" });
        // Fetch fresh data without causing redirect
        setTimeout(() => fetchAllData(), 500);
      } else {
        toast({ title: "Error", description: "Failed to approve internship", variant: "destructive" });
      }
    } catch (error) {
      toast({ title: "Error", description: "Failed to approve internship", variant: "destructive" });
    }
  ...
  const rejectInternship = async (id: number, event?: React.MouseEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "Success", description: "Internship rejected successfully" });
        setTimeout(() => fetchAllData(), 500);
      } else {
        toast({ title: "Error", description: "Failed to reject internship", variant: "destructive" });
      }
    } catch (error) {
      toast({ title: "Error", description: "Failed to reject internship", variant: "destructive" });
    }
  ...
  const changeAdminCredentials = async () => {
    ...
    if (!adminForm.newEmail && !adminForm.newPassword) {
      toast({ title: "Error", description: "Please enter a new email or password to update", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        toast({ title: "Success", description: "Admin credentials updated successfully" });
        setAdminForm({ currentPassword: '', newEmail: '', newPassword: '', confirmPassword: '' });
        setShowAdminSettings(false);
      } else {
        const errorData = await response.json();
        toast({ title: "Error", description: errorData.message || "Failed to update credentials", variant: "destructive" });
      }
    } catch (error) {
  ...
  const approveAllPendingInternships = async () => {
    const pendingInternships = internships.filter(internship => !internship.isApproved && internship.status === 'pending');
    if (pendingInternships.length === 0) {
      toast({ title: "No Action Needed", description: "No pending internships to approve" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Error", description: "Failed to approve internships", variant: "destructive" });
    }
  };
  ...
  const createBenefit = async () => {
    try {
      ...
      Object.keys(benefitForm).forEach(key => {
        formData.append(key, benefitForm[key as keyof typeof benefitForm] as string);
      });
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center py-8">
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-300">Loading admin data...</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Tabs defaultValue="jobs" className="w-full">
          <TabsList className="grid w-full grid-cols-11 bg-gray-100 border border-gray-200">
            <TabsTrigger value="jobs" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Jobs</TabsTrigger>
            <TabsTrigger value="applications" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Applications</TabsTrigger>
            <TabsTrigger value="cvs" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">CVs</TabsTrigger>
            <TabsTrigger value="matching" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">CV-Job Match</TabsTrigger>
            <TabsTrigger value="directory" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Directory</TabsTrigger>
            <TabsTrigger value="whitelist" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Whitelist</TabsTrigger>
            <TabsTrigger value="access" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Access</TabsTrigger>
            <TabsTrigger value="internships" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Internships</TabsTrigger>
            <TabsTrigger value="courses" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Courses</TabsTrigger>
            <TabsTrigger value="benefits" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Benefits</TabsTrigger>
            <TabsTrigger value="backup" className="text-gray-800 data-[state=active]:bg-white data-[state=active]:text-blue-600">Backup</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="jobs" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                <CardHeader>
                  <CardTitle className="text-gray-900 dark:text-white">Create New Job</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <Button onClick={createJob} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle className="text-gray-900">All Jobs ({jobs.length})</CardTitle>
                </CardHeader>
                <CardContent>
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <div className="flex gap-2 mt-2">
                                <Badge variant={job.isApproved ? "default" : "secondary"} className={job.isApproved ? "bg-green-100 text-green-800 border-green-300" : "bg-yellow-100 text-yellow-800 border-yellow-300"}>
                                  {job.isApproved ? 'Approved' : 'Pending'}
                                </Badge>
                                {job.industry && <Badge variant="outline" className="text-blue-700 border-blue-300 bg-blue-50">{job.industry}</Badge>}
                                {job.location && <Badge variant="outline" className="text-purple-700 border-purple-300 bg-purple-50">{job.location}</Badge>}
                                {job.salaryRange && <Badge variant="outline" className="text-green-700 border-green-300 bg-green-50">{job.salaryRange}</Badge>}
                              </div>
                              {job.description && (
              ...
              <CardContent>
                ...
                <div className="flex flex-wrap gap-4 mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <div className="flex-1 min-w-[200px]">
                    <Label htmlFor="application-search" className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      Search Applications
                    </Label>
                    <Input
                  ...
                  <div className="min-w-[150px]">
                    <Label htmlFor="position-type-filter" className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      Position Type
                    </Label>
                  ...
                  <div className="min-w-[150px]">
                    <Label htmlFor="sector-filter" className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      Sector
                    </Label>
                  ...
                  {(applicationSearch || positionTypeFilter !== 'all' || statusFilter !== 'all' || sectorFilter !== 'all') && (
                  ...
                  {(() => {
                    ...
                    const filteredApplications = applications.filter((application) => {
                      ...
                      const matchesSearch = !applicationSearch || 
                        ...
                        (application.company && application.company.toLowerCase().includes(searchLower));

                      // Position type filter
                      const matchesPositionType = positionTypeFilter === 'all' ||
                        (positionTypeFilter === 'job' && application.jobId) ||
                        (positionTypeFilter === 'internship' && application.internshipId);
                      ...
                      return matchesSearch && matchesPositionType && matchesStatus && matchesSector;
                    });
                      ...
                      return (
                        <div className="text-center py-8">
                          ...
                          <p className="text-muted-foreground">
                            {applications.length === 0 ? "No applications submitted yet" : "No applications match the current filters"}
                          </p>
                        </div>
                      );
                    }
                    ...
                    return filteredApplications.map((application) => (
                      <div key={application.id} className="border border-border rounded-lg p-4 bg-muted">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3 text-sm">
                              ...
                              {application.applicantPhone && (
                                <div>
                                  ...
                                  <span className="ml-2 text-green-600 hover:underline cursor-pointer"
                          ...
                          <div className="flex flex-col gap-2 ml-4 min-w-[200px]">
                            ...
                            <div className="flex flex-col gap-1">
                              {application.cvFileName && application.cvFilePath ? (
                                <>
                                  <Button
                                    onClick={() => {
                                      // Extract the actual filename from the file path
                                      const actualFilename = application.cvFilePath?.split('/').pop();
                                      if (actualFilename) {
                                        window.open(`/api/cv/download/${encodeURIComponent(actualFilename)}`, '_blank');
                                      }
                                    }}
                                    size="sm"
                                    variant="outline"
                                  ...
                                  <Button
                                    onClick={() => application.cvFilePath && downloadCV(application.cvFilePath, application.cvFileName)}
                                    size="sm"
                                    variant="outline"
          ...
          <TabsContent value="matching" className="space-y-6">
            <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
              <CardHeader>
                <CardTitle className="text-gray-900 dark:text-white">CV-Job Matching System</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Available CVs</h3>
                    <div className="space-y-3">
                      {cvs.length === 0 ? (
                        <div className="text-center py-8 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700">
                          ...
                          <p className="text-gray-500 dark:text-gray-400">No CVs available for matching</p>
                        </div>
                      ) : (
                        cvs.map((cv) => (
                          <div key={cv.id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
                            <h4 className="font-medium text-gray-900 dark:text-white">{cv.name}</h4>
                            <p className="text-sm text-gray-600 dark:text-gray-300">{cv.section} • {cv.experience}</p>
                            {cv.skills && (
                              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Skills: {cv.skills}</p>
                            )}
                            <div className="flex gap-2 mt-2">
                              <Button
                                size="sm"
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Matching Results</h3>
                    {matchingResults.length > 0 ? (
                        ...
                        {matchingResults.map((match, index) => (
                          <div key={index} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-green-50 dark:bg-green-900/20">
                            <div className="flex justify-between items-start mb-2">
                              <h4 className="font-medium text-gray-900 dark:text-white">{match.job?.title}</h4>
                              <Badge 
                                variant="outline" 
                                className={`${
                                  match.score >= 80 ? 'bg-green-100 text-green-800 border-green-300 dark:bg-green-900 dark:text-green-200' :
                                  match.score >= 60 ? 'bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-900 dark:text-yellow-200' :
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-300">{match.job?.company}</p>
                            {match.job?.industry && (
                              <p className="text-sm text-gray-500 dark:text-gray-400">Industry: {match.job.industry}</p>
                            )}
                            {match.job?.experienceLevel && (
                              <p className="text-sm text-gray-500 dark:text-gray-400">Level: {match.job.experienceLevel}</p>
                            )}
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="text-center py-8 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">Click "Find Jobs" to see matches</p>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <TabsContent value="cvs" className="space-y-6">
            <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
              <CardHeader>
                <CardTitle className="text-gray-900 dark:text-white">CV Management ({cvs.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {cvs.map((cv) => (
                    <div key={cv.id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
                      <h3 className="font-semibold text-gray-900 dark:text-white">{cv.name}</h3>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">{cv.experience}</p>
                      <div className="flex gap-2 mt-3">
                        {cv.cvFileName && (
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => window.open(`/api/cv-showcase/${cv.id}/download`, '_blank')}
          ...
          <TabsContent value="directory" className="space-y-6">
            <Card className="bg-white border-gray-200">
              <CardHeader>
                <CardTitle className="text-gray-900">Professional Directory ({profiles.length})</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {profiles.length === 0 ? (
                      ...
                      <div key={profile.id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            ...
                            <div className="flex gap-2 mt-2">
                              {profile.company && <Badge variant="outline" className="border-gray-400 dark:border-gray-500 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-600">{profile.company}</Badge>}
                              {profile.position && <Badge variant="outline" className="border-gray-400 dark:border-gray-500 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-600">{profile.position}</Badge>}
                              {profile.industry && <Badge variant="outline" className="border-gray-400 dark:border-gray-500 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-600">{profile.industry}</Badge>}
                              <Badge variant={profile.isVisible ? "default" : "secondary"} className={profile.isVisible ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200"}>
          ...
          <TabsContent value="whitelist" className="space-y-6">
            <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
              <CardHeader>
                <CardTitle className="text-gray-900 dark:text-white">Email Whitelist Management</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4 mb-4">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="bg-white dark:bg-gray-700 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600"
                    />
                    <Button onClick={addToWhitelist} className="bg-blue-600 hover:bg-blue-700 text-white">
                      <Plus className="h-4 w-4 mr-2" />
                      Add Email
                    </Button>
                  </div>
                ...
                <div className="space-y-3">
                  {whitelist
                    ...
                    .map((item) => (
                    <div key={item.id} className="border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 p-4">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                          ...
                          {item.name && (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-2 text-sm">
                              <div>
                                ...
                                <span className="ml-2 text-gray-900 dark:text-white font-medium">{item.name}</span>
                              </div>
                              {item.unit && (
                                <div>
                                  <span className="text-gray-600 dark:text-gray-400">Unit:</span>
                                  <span className="ml-2 text-gray-900 dark:text-white font-medium">{item.unit}</span>
                                </div>
                              )}
                              {item.phone && (
                                <div>
                                  <span className="text-gray-600 dark:text-gray-400">Mobile:</span>
                                  <span className="ml-2 text-gray-900 dark:text-white font-medium">{item.phone}</span>
                                </div>
                              )}
                            </div>
                          )}
                ...
                <div className="space-y-4">
                  {accessRequests.map((request) => (
                    <div key={request.id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-700">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <h3 className="font-semibold text-gray-900 dark:text-white">{request.fullName}</h3>
                          <p className="text-sm text-gray-600 dark:text-gray-300">{request.email}</p>
                          <p className="text-sm text-gray-700 dark:text-gray-200">Unit: {request.unitNumber}</p>
                          {request.mobile && <p className="text-sm text-gray-700 dark:text-gray-200">Mobile: {request.mobile}</p>}
          ...
          <TabsContent value="courses" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle className="text-gray-900">Create New Course</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                  ...
                  <Button onClick={createCourse} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
                    <Plus className="h-4 w-4 mr-2" />
                    Create Course
                  </Button>
                </CardContent>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white border-gray-200">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button 
                      onClick={editingInternship ? updateInternship : createInternship} 
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <div className="flex justify-between items-center">
                    <CardTitle className="text-gray-900">All Internships ({internships.length})</CardTitle>
                    {internships.filter(internship => !internship.isApproved && internship.status === 'pending').length > 0 && (
                      <Button
                        onClick={approveAllPendingInternships}
                        className="bg-green-600 hover:bg-green-700 text-white"
                      >
                        <Check className="h-4 w-4 mr-2" />
                        Approve All ({internships.filter(internship => !internship.isApproved && internship.status === 'pending').length})
                      </Button>
                    )}
                  </div>
                </CardHeader>
                        ...
                        <div key={internship.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <div className="flex gap-2 mt-2">
                                <Badge variant={internship.isApproved ? "default" : "secondary"}>
                                  {internship.isApproved ? 'Approved' : 'Pending'}
                                </Badge>
                                {internship.department && <Badge variant="outline">{internship.department}</Badge>}
                                {internship.duration && <Badge variant="outline">{internship.duration}</Badge>}
                              </div>
                            </div>
                            <div className="flex gap-2 ml-4">
                              <Button
                              ...
                              <Button
                                onClick={() => editApprovedInternship(internship)}
                                size="sm"
                                variant="outline"
                              ...
                              {!internship.isApproved && internship.status === 'pending' && (
                                <>
                                  <Button
                                    onClick={(e) => approveInternship(internship.id, e)}
                                    size="sm"
                                    className="bg-green-600 hover:bg-green-700 text-white"
                                  >
                                    <Check className="h-4 w-4 mr-1" />
                                  ...
                                  <Button
                                    onClick={(e) => rejectInternship(internship.id, e)}
                                    size="sm"
                                    variant="outline"
                ...
                <CardContent>
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    {approvedInternships.length === 0 ? (
                      <div className="text-center py-8">
                        <GraduationCap className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                        <p className="text-gray-500">No published internships yet</p>
                      </div>
                    ) : (
                        ...
                        <div key={internship.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="flex gap-2 ml-4">
                              ...
                              <Button
                                onClick={() => {
                                  if (confirm(`Delete "${internship.title}" internship? This will remove it from public view.`)) {
          ...
          <TabsContent value="benefits" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle className="text-gray-900">Create New Community Benefit</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor="showOnHomepage" className="text-gray-700">Show on Homepage</Label>
                    </div>
                  </div>
                  <Button onClick={createBenefit} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
                    <Plus className="h-4 w-4 mr-2" />
                    Create Benefit
                  </Button>
                </CardContent>
              ...
              <Card className="bg-white border-gray-200">
                <CardHeader>
                  <CardTitle className="text-gray-900">All Benefits ({benefits.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    {benefits.length === 0 ? (
          ...
          <TabsContent value="backup" className="space-y-6">
            <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
              <CardHeader>
                <CardTitle className="text-gray-900 dark:text-white">System Backup & Restore</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                {/* Create Backup Section */}
                <div className="border-b pb-4">
                  <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">Create New Backup</h3>
                  <div className="flex gap-3">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center">
                      Creates a complete backup of all platform data including jobs, CVs, profiles, and files
                    </p>
                  </div>
                </div>
                ...
                {/* Backup History */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">Backup History</h3>
                  {backups.length > 0 ? (
                    <div className="space-y-3">
                      {backups.map((backup) => (
                        <div key={backup.id} className="flex items-center justify-between p-4 border rounded-lg">
                          <div>
                            <p className="font-medium text-gray-900 dark:text-white">{backup.description}</p>
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              Created: {formatDateTime(backup.timestamp)} • Size: {backup.size} • Files: {backup.filesCount}
                            </p>
                    ...
                    <p className="text-gray-600 dark:text-gray-400">No backups available. Create your first backup above.</p>
                  )}
                </div>
              </CardContent>
            </Card>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-gray-900 font-semibold">{selectedJob.title}</p>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-700">Company</Label>
                          <p className="text-gray-900 font-semibold">{selectedJob.company}</p>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-700">Industry</Label>
                          <p className="text-gray-900">{selectedJob.industry || 'Not specified'}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-gray-900">{selectedJob.experienceLevel || 'Not specified'}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-gray-900">{selectedJob.salaryRange || 'Not disclosed'}</p>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-700">Status</Label>
                          <Badge variant={selectedJob.isApproved ? "default" : "secondary"} className={selectedJob.isApproved ? "bg-green-100 text-green-800 border-green-300" : "bg-yellow-100 text-yellow-800 border-yellow-300"}>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-blue-600 font-medium">{selectedJob.contactEmail}</p>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-gray-700">Contact Phone</Label>
                          <p className="text-gray-900 font-medium">{selectedJob.contactPhone}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                      ...
                      {jobPoster ? (
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-gray-900 font-semibold">{jobPoster.name || 'Not provided'}</p>
                          </div>
                          <div>
                            <Label className="text-sm font-medium text-gray-700">Email</Label>
                            <p className="text-blue-600 font-medium">{jobPoster.email}</p>
                          </div>
                          {jobPoster.phone && (
                            <div>
                              <Label className="text-sm font-medium text-gray-700">Phone</Label>
                              <p className="text-gray-900 font-medium">{jobPoster.phone}</p>
                            </div>
                          )}
                          {jobPoster.unit && (
                            <div>
                              <Label className="text-sm font-medium text-gray-700">Unit Number</Label>
                              <p className="text-gray-900 font-medium">{jobPoster.unit}</p>
                            </div>
                          )}
                          <div>
                            <Label className="text-sm font-medium text-gray-700">User ID</Label>
                            <p className="text-gray-600 font-medium">#{jobPoster.id}</p>
                          </div>
                          {jobPoster.createdAt && (
                            <div>
                              <Label className="text-sm font-medium text-gray-700">Member Since</Label>
                              <p className="text-gray-900 font-medium">{formatDate(jobPoster.createdAt)}</p>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="text-center py-4">
                          <p className="text-gray-600 font-medium">No poster information available</p>
                          <p className="text-sm text-gray-500 mt-1">Job created by admin</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          <div>
                            ...
                            <p className="text-gray-900 font-medium">{formatDate(selectedJob.createdAt)}</p>
                          </div>
                          {selectedJob.updatedAt && selectedJob.updatedAt !== selectedJob.createdAt && (
                            <div>
                              <Label className="text-sm font-medium text-gray-700">Last Updated</Label>
                              <p className="text-gray-900 font-medium">{formatDate(selectedJob.updatedAt)}</p>
                            </div>
                          )}
                        </div>
                      </CardContent>
        ...
        {editingJob && (
          <Dialog open={!!editingJob} onOpenChange={() => setEditingJob(null)}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white border-gray-300">
              <DialogHeader>
                <DialogTitle className="text-gray-900">Edit Job: {editingJob.title}</DialogTitle>
              </DialogHeader>
              ...
              <div className="flex gap-3 mt-6">
                <Button onClick={updateJob} className="bg-orange-600 hover:bg-orange-700 text-white">
                  <Save className="h-4 w-4 mr-2" />
                  Save Changes
                </Button>
                <Button onClick={() => setEditingJob(null)} variant="outline" className="border-gray-300 text-gray-700">
                  Cancel
                </Button>
              </div>
            </DialogContent>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="h-4 w-4"
                  />
                  <Label htmlFor="isActive" className="text-gray-700">Active User</Label>
                </div>
              </div>
              
              <div className="flex gap-3 mt-6">
                <Button onClick={updateUser} className="bg-blue-600 hover:bg-blue-700 text-white">
                  <Save className="h-4 w-4 mr-2" />
                  Save Changes
                </Button>
                <Button onClick={() => setEditingUser(null)} variant="outline" className="border-gray-300 text-gray-700">
                  Cancel
                </Button>
              </div>
            </DialogContent>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900 bg-gray-50 p-2 rounded border">{selectedInternship.title}</p>
                  </div>
                  <div>
                    <Label className="text-gray-700 font-medium">Company</Label>
                    <p className="text-gray-900 bg-gray-50 p-2 rounded border">{selectedInternship.company}</p>
                  </div>
                </div>

                <div>
                  <Label className="text-gray-700 font-medium">Description</Label>
                  <p className="text-gray-900 bg-gray-50 p-3 rounded border whitespace-pre-wrap">{selectedInternship.description}</p>
                </div>

                {selectedInternship.requirements && (
                  <div>
                    <Label className="text-gray-700 font-medium">Requirements</Label>
                    <p className="text-gray-900 bg-gray-50 p-3 rounded border whitespace-pre-wrap">{selectedInternship.requirements}</p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedInternship.department && (
                    <div>
                      ...
                      <p className="text-gray-900 bg-gray-50 p-2 rounded border">{selectedInternship.department}</p>
                    </div>
                  )}
                  {selectedInternship.duration && (
                    <div>
                      <Label className="text-gray-700 font-medium">Duration</Label>
                      <p className="text-gray-900 bg-gray-50 p-2 rounded border">{selectedInternship.duration}</p>
                    </div>
                  )}
                </div>

                <div className="flex items-center gap-4 pt-2">
                  <Badge variant={selectedInternship.isApproved ? "default" : "secondary"}>
                    {selectedInternship.isApproved ? 'Approved' : 'Pending Review'}
                  </Badge>
                ...
                {!selectedInternship.isApproved && selectedInternship.status === 'pending' && (
        ...
        {showAdminSettings && (
          <Dialog open={showAdminSettings} onOpenChange={setShowAdminSettings}>
            <DialogContent className="max-w-lg bg-white border-gray-200">
              ...
              <div className="space-y-4">
                <div>
                  <Label className="text-gray-700">Current Password (Required)</Label>
                  <Input
                    type="password"
                ...
                <div className="border-t pt-4">
                  <Label className="text-gray-700">New Admin Email (Optional)</Label>
                  <Input
                    type="email"
                ...
                <div className="flex gap-3 mt-6">
                  <Button 
                    ...
                    className="bg-green-600 hover:bg-green-700 text-white flex-1"
                    disabled={!adminForm.currentPassword || (!adminForm.newEmail && !adminForm.newPassword)}
                  >
                    <Save className="h-4 w-4 mr-2" />
                    Update Settings
                  </Button>
